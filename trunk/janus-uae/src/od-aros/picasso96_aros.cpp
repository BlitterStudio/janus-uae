...
static void DumpTemplate (struct Template *tmp, unsigned long w, unsigned long h)
{
	...
	width = (w + 7) >> 3;
	write_log (_T("mem: %p, xoffset = %d, bpr = %d\n"), mem, tmp->XOffset, tmp->BytesPerRow);
	for (row = 0; row < h; row++) {
  ...
  DebOut("rgbformat %d, pixbytes %d\n", rgbformat, pixbytes);
    ...
    DebOut("here\n");
      ...
      DebOut("here, too\n");
			...
			v = RGBFB_B8G8R8A8_32;
      DebOut("ERROR: what happens here!? (d=%d)\n", d);
    }
		break;
	case RGBFB_R8G8B8A8:
      DebOut("here\n");
...
static uae_u32 REGPARAM2 picasso_FindCard (TrapContext *ctx)
{
  ...
  DebOut("uaegfx_base: %lx, uaegfx_active %d, gfxmem_bank.start %lx\n", uaegfx_base, uaegfx_active, gfxmem_bank.start);
...
static void CopyLibResolutionStructureU2A (struct LibResolution *libres, uaecptr amigamemptr)
{
	...
	for (i = 0; i < PSSO_LibResolution_sizeof; i++)
		put_byte (amigamemptr + i, 0);
  DebOut("libres->Name: %s\n", libres->Name);
  DebOut("libres->P96ID: %s\n", libres->P96ID);
...
void add_mode (struct MultiDisplay *md, int w, int h, int d, int freq, int lace) {
	...
	*/
	...
	if (d == 32)
		ct = RGBMASK_32BIT;
	if (ct == 0) {
    DebOut("WARNING: ct == 0, mode ignored!\n");
		return;
  }
	d /= 8;
	i = 0;
  ...
  DebOut("add_mode: md->DisplayModes[i].res.width %d, w %d\n", md->DisplayModes[i].res.width,w);
...
static int picasso96_alloc2_size=-1;
...
static uae_u32 REGPARAM2 picasso_InitCard (TrapContext *ctx)
{
  ...
  printf("picasso_InitCard!!\n");
	...
	while (newmodes[i].depth >= 0) {
		...
		TCHAR *s;
		j = i;
    DebOut("call addmode(width %d, height %d)\n", newmodes[i].res.width, newmodes[i].res.height);
    ...
    DebOut("amem %lx + PSSO_LibResolution_sizeof %lx = %lx (picasso96_amemend %lx)\n", amem, PSSO_LibResolution_sizeof, PSSO_LibResolution_sizeof + amem, picasso96_amemend);
	...
	if (amem > picasso96_amemend) {
		...
		DebOut ("P96: display resolution list corruption %08x > %08x (%d)\n", amem, picasso96_amemend, i);
  }
...
#else
	write_log (_T("RTGFREQ: %d*%d = %d / %d = %d\n"), maxvpos_nom, (int) vblank_hz, (int) (maxvpos_nom * vblank_hz), (int) p96vblank, p96syncrate);
#endif
}
	...
	unsigned long width, unsigned long height, uae_u8 mask, struct ColorIndexMapping *cim)
{
  ...
  DebOut("ri %p, bm %p, srcx %d, srcy %d, dstx %d, dsty %d, width %d, height %d, mask %d, cim %p\n", ri, bm, srcx, srcy, dstx, dsty, width, height, mask, cim);
    ...
    DebOut("rows: %d\n", rows);
      ...
      DebOut("bpp: %d\n", bpp);
			...
			case 4:
        DebOut("image2: %p\n", image2);
        DebOut("cim->Colors[v]: %d\n", cim->Colors[v]);
				((uae_u32 *)image2)[0] = cim->Colors[v];
				image2 += 4;
				break;
      default:
        DebOut("ERROR: unsupported bpp %d !?!?!\n", bpp);
			}
			bitoffs--;
			bitoffs &= 7;
      DebOut("bitoffs: %d\n", bitoffs);
...
static void copyrow (uae_u8 *src, uae_u8 *dst, int x, int y, int width, int srcbytesperrow, int srcpixbytes, int dstbytesperrow, int dstpixbytes, bool direct, int convert_mode)
{
	...
	if (currprefs.gfx_api) {
		switch (convert_mode)
		{
			...
			return;
      default:
        DebOut("unsupported convert_mode: %d\n", convert_mode);
		}
	} else {
		switch (convert_mode)
		{
...
#warning TODO
  ...
  DebOut("WARNING: force picasso_vidinfo.pixbytes = 4!!\n");
