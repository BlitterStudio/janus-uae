...
#if 0
/* no, it is defined in jit/compemu_prefs.cpp .. */
...
static uaecptr ShowEA (void *f, uaecptr pc, uae_u16 opcode, int reg, unsigned int mode, unsigned int size, TCHAR *buf, uae_u32 *eaddr, int safemode)
{
	uae_u16 dp;
	uae_s8 disp8;
	uae_s16 disp16;
...
/* SetSR clashes with AROS builtin funtion SetSR,
   but SetSR seems not to be used anywhere anyhow?
 */
static void uae_SetSR (uae_u16 sr)
{
	regs.sr &= 0xff00;
	regs.sr |= sr;
