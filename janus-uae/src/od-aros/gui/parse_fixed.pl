...
sub gen_line($$) {
  ...
  switch($type) {
    ...
    case "CONTROL" {
      ...
      $debug && print "    }\n";
      push @idc, $attr[1];
      printf(CPPFILE  "  { 0, %-20s, NULL, %-11s, %3d, %3d, %3d, %3d, %s, %s, 0x%08lx, %d },\n", $attr[1], $type, $attr[4], $attr[5]+$plus_height, $attr[6], $attr[7], get_text($attr[0]), get_help($attr[0]), parse_flags($attr[3]), parse_flags2($attr[3]));
    }
    case "PUSHBUTTON" {
      $debug && print "  = $type {\n";
      $debug && print "      string: ".$attr[0]."\n";
  ...
  if($line =~ /^;/) {
    ...
    if($idd_active) {
      ...
      if($line =~ /^END/) {
        ...
        # finish!
        print CPPFILE "  { 0, 0, NULL, 0,            0,   0,   0,   0,  NULL, 0 }\n";
        print CPPFILE "};\n\n";
      }
      else {
