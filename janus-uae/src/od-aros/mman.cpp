...
DebOut("changed_prefs.z3_mapping_mode: %d\n", changed_prefs.z3_mapping_mode);
DebOut("Z3MAPPING_UAE: %d\n", Z3MAPPING_UAE);
DebOut("cpuboard_memorytype(&changed_prefs): %d\n", cpuboard_memorytype(&changed_prefs));
DebOut("BOARD_MEMORY_BLIZZARD_12xx: %d\n", BOARD_MEMORY_BLIZZARD_12xx);
  ...
  DebOut("changed_prefs.rtgmem_size: %d\n", changed_prefs.rtgmem_size);
	if (p96base_offset) {
      ...
      DebOut("changed_prefs.rtgmem_size: %d\n", changed_prefs.rtgmem_size);
			DebOut(_T("RTG memory is not supported in this configuration."));
